...
class AttentionBoundingBoxLoss(nn.Module):
    ...
    # Modificata la funzione per usare DBSCAN per estrarre il bounding box dall'attenzione
    def attention_to_box(self, attention_maps, threshold=0.5, eps=2, min_samples=5):
        ...
        """
        attention_map = attention_maps.mean(dim=1).squeeze().cpu().numpy()  # Media su tutte le heads
        ...
        if coords.shape[0] == 0:
            # Se non ci sono punti sopra la soglia, ritorna un bounding box vuoto
        ...
        if largest_cluster_label == -1:
            return torch.tensor([[0, 0, 0, 0]], dtype=torch.float32)  # Se il cluster più grande è rumore

        # Ottieni le coordinate del cluster più grande
        largest_cluster_coords = coords[clustering.labels_ == largest_cluster_label]
        x_min, y_min = largest_cluster_coords.min(axis=0)
        x_max, y_max = largest_cluster_coords.max(axis=0)
...
epochs = 1  #modificato per vedere prima sse funziona
best_val_acc = 0.0  
metrics = {
    "epoch": [],
    "train_loss": [],
...
# Visualizzazione e calcolo delle metriche
with torch.no_grad():
    model.eval()
    count = 0
    for images, bounding_boxes, labels in test_loader:
        ...
        logits, attention_maps = model(images)
        pred_bboxes = criterion.attention_to_box(attention_maps).to(device)  # Genera i predetti bounding box dalle attention maps
